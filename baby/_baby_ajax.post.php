...
<div id="cr_c">
	...
	<div class="problem_bannerr" <?php if($i % 2 == 0){echo 'style="background:#D2D8E4;"';}?>>
		<div style="width:150px;  margin-left:30px; font-weight:bold; float:left; display: inline;"><a href="/test/test.php?id=<?php echo $problem->problem_id;?>"><?php echo $problem->name; ?></a></div>
		<div style="font-size:12px; font-weight:100px;">测评时间：<?php echo $problem->created_at; ?></div>
		<div style="margin-left:80px; float:left;"><a href="/test/test_result.php?test_id=<?php echo $problem->problem_id;?>" <?php if($i % 2 == 0){echo 'style="color:#333333;"';}?>>测试结果报表</a></div>
		<div style="margin-left:80px; float:left;"><a href=" /test/review.php?id=<?php echo $problem->problem_id;?>" <?php if($i % 2 == 0){echo 'style="color:#333333;"';}?>>测试回顾</a></div>
	</div>
	<?php $i++; }?>
</div>
<?php 		
