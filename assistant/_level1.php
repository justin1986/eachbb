			...
			<div class="resultt_banner">
				...
				$i=0;
				 foreach($display->assistants as $j => $assistant){
				 	?>
				<div class="result_title"<?php if($j==0 || $j == 1){ if($i % 2 ==1){ echo 'style="margin-top:10px; margin-left:3px"';}else{?>style="margin-top:10px;"<?php }}?>><a href="/assistant/assistant.php?id=<?php echo $assistant[id];?>" title="<?php echo $assistant[title];?>"><?php echo mb_substr($assistant[title],0,12,'utf-8');?><?php  if(mb_strlen($assistant[title],'utf-8') > 12){ echo "...";}?></a></div>
				<?php $i++; }?>
			</div>
