<?php
	...
	if(!$user) die();
	$result= $db->query("SELECT * FROM eachbb_member.lastest_news where u_id in (SELECT f_id FROM friend where u_id ={$user->id} group by u_id) order by rand(),created_at desc LIMIT 10");
    if($result){
